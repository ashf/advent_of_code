...
class Hand : IComparable<Hand>
{
    ...
    HandType CalcHandType()
    {
        ...
        var jCount = _cards.Count(c => c == 'J');
        
        if (jCount == 5) return HandType.FiveOfAKind;

        var orderedCardCount = cardCount.OrderByDescending(x => x.Value);
        var first = orderedCardCount.First();
        
        if ((first.Value + jCount) == 5) return HandType.FiveOfAKind;
        if ((first.Value + jCount) == 4) return HandType.FourOfAKind;
        if ((first.Value + jCount) == 1) return HandType.HighCard;
        
        var second = orderedCardCount.Skip(1).First();

        if ((first.Value + jCount) == 3)
        {
